...
/** estado para la máquina de estados del parser de comandos */
typedef enum {
	CMD_IDLE = 0,
	CMD_RECEIVING,
	CMD_PROCESS,
...
/**Buffer para recibir datos por UART */
static uint8_t uartRxBuffer[CMD_MAX_LINE];

/**Máquina de estados del parser de comandos */
static cmdParserState_t cmdParserStateFsm = CMD_IDLE;

/**Variables para almacenar los argumentos actuales y el handler del comando a ejecutar */
static char* currentArgv[CMD_MAX_TOKENS];

/**Variable para almacenar el número de argumentos actuales */
static uint8_t currentArgc = 0;

/*Variable para almacenar el handler del comando a ejecutar */
static command_handler_t currentCmdHandler;

/*Acción para el comando HELP */
static void helpAction(uint8_t argc, char *argv[])
{
	cmdPrintHelp();
}

/**Acción para el comando LED */
...
/**Función para tokenizar la línea de comando recibida
 * input: La línea de comando a tokenizar
 * argv: El array donde se almacenarán los tokens
...
/**Función para ejecutar la acción correspondiente a un error de comando
 * errorAction: El tipo de error ocurrido
 */
...
/** Función para procesar la línea de comando recibida y ejecutar la acción correspondiente
